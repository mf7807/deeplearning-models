...
def asymmetric_quantize_tensor(tensor):
    min_val = tensor.min()
    max_val = tensor.max()
    ...
    q = torch.round(tensor / scale + zero_point).clamp(0, 255)
    return (q - zero_point) * scale

# Apply asymmetric quantization to the model
def apply_asymmetric(model):
    for name, param in model.named_parameters():
        if "weight" in name:
            param.data = asymmetric_quantize_tensor(param.data)
    return model

model = apply_asymmetric(model)
