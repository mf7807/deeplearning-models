...
def symmetric_quantize_tensor(tensor):
    max_val = tensor.abs().max()
    scale = max_val / 127
    q = torch.round(tensor / scale).clamp(-127, 127)
    return q * scale

# Apply symmetric quantization to the model
def apply_symmetric(model):
    for name, param in model.named_parameters():
        if "weight" in name:
            param.data = symmetric_quantize_tensor(param.data)
    return model

model = apply_symmetric(model)
